...
function sid1(a,b,c,d){
	ley.fillStyle="green";
	ley.fillRect(a,b,c,d);
}
function sid2(a,b,c,d){
	ley.fillStyle="yellow";
	ley.fillRect(a,b,c,d);
}
function sid3(a,b,c,d){
	ley.fillStyle="red";
	ley.fillRect(a,b,c,d);
}
...
var collector=[]; // empty array to control data
   /*function many that invokes function Addone according to number of input with loop*/
	...
	function addone(){
		...
		holder.setAttribute( "id", "inputId");
		holder .focus();
	...
	function compute(){
		...
		rel.width= (collector.length+1)*60;
		w=rel.width;rel.height=340;
		h=rel.height;
		...
		} //max is discovered from the array to be used for scaling
		...
		for (var i = 0; i< collector.length; i++ ) { 
			if (collector[i]>=0){
				var scale= ((300/max)*0.95), b= collector[i]*scale; 
			 	if (b>((2/3)*(300))){
					perf.fillStyle="green";
					perf.fillRect(j,h-40,50,-b);
				}//the color section with data value
				if (b>=((1/3)*(300)) && b<=((2/3)*(300))){
					perf.fillStyle="yellow";
					perf.fillRect(j,h-40,50,-b);
				} //the color section with data value
				else{
					if (b<((1/3)*(300))) {
	    ...
	    //text are written and positioned below each bar			
			perf.strokeStyle="white";
			perf.font="17px century gothic";
			perf.strokeText(collector[i],j+10,h-20);
			j=j+60;
			} else{
				alert('Please enter a valid number in box numer '+(i+1));
			} 
		}
		console.log(max);
		console.log(collector);
...
function perf1(a,b,c,d){
	perf.fillStyle="green";
	perf.fillRect(a,b,c,d);
}
function perf2(a,b,c,d){
	perf.fillStyle="yellow";
	perf.fillRect(a,b,c,d);
}
function perf3(a,b,c,d){
	perf.fillStyle="red";
	perf.fillRect(a,b,c,d);
}
